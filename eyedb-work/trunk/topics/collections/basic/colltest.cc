...
perform_get(Database &db)
{
  ...
  p = new Person(&db);
  p->setName("array 1");
  c->getLitCRefArrColl()->insertAt(0, p);
  p->release();

  p = new Person(&db);
  p->setName("set 1");
  c->getLitCRefColl()->insert(p);
  p->release();

  c->store(RecMode::FullRecurs);

  c->getLitCRefArrColl()->retrieveAt(0, (Object *&)p);
  std::cout << (void *)p << " " << p->getName() << " " << p->getRefCount() << std::endl;
  ...
  c->getLitCRefColl()->getElements(obj_arr);
  for (int n = 0; n < obj_arr.getCount(); n++) {
    p = dynamic_cast<Person *>(obj_arr[n]);
    std::cout << (void *)p << " " << p->getName() << " " << p->getRefCount() << std::endl;
  }
