...
cls->setAttributes((Attribute **)class_info[Basic_Type].items, \
  ...
  */
	    ...
	    const char *ext1 = "", Bool check_if_exists = False)
  {
	...
	if (stat(file, &s) >= 0)
	  {
	    fprintf(stderr, "warning, file '%s' exists: does not overwrite it.\n",
		    file);
	    if (!fdnull)
  ...
  skel_comment(FILE *fd, const char *files, const char *febe, Bool backend)
  {
    ...
    fprintf(fd,
	    ...
	    "cp %s%s%s.so $EYEDBROOT/$EYEDBARCH/etc/so/.\n"
	    "\n"
	    "#5. Change the file access mode\n"
	    "chmod a+r $EYEDBROOT/$EYEDBARCH/etc/so/%s%s%s.so\n",
	    files, febe, s,
	    files, febe, s);
  ...
  Schema::genODL(FILE *fd, unsigned int flags) const
  {
    ...
    if (name && *name)
      {
	fprintf(fd ,"\n//\n");
	fprintf(fd, "// EyeDB Version %s Copyright (c) 1995-2005 SYSRA\n",
		eyedb::getVersion());
	fprintf(fd, "//\n");
	fprintf(fd, "// %s Schema\n", name);
	fprintf(fd, "//\n");
	time_t t;
	time(&t);
	fprintf(fd, "// Automatically Generated by eyedbodl at %s", ctime(&t));
	fprintf(fd, "//\n\n");
			  ...
			  const char *dirname, const char *package)
  {
	...
	if (fd)
	  {
	    head_gen_C(fd, pfile, "", package, False);
	    fclose(fd);
	  }
  ...
  make_java_file(const char *dirname, const char *package,
		 const char *prefix, const char *name, FILE *&fd)
  {
    char file[256];
			 ...
			 LinkedList *qseq_list)
  {
    ...
    if (status = make_files(hints.dirname, hints.fileprefix, package,
			    ...
			    &fdmthfe, &fdmthbe, &fdmk))
      return status;

    head_gen_C(fdh, hints.fileprefix, hints.h_suffix, package, True);
    ...
    head_gen_C(fdc, hints.fileprefix, hints.c_suffix, package, True);
    ...
    */
    ...
    /*
      ...
      hints.c_suffix);
    */
    head_gen_C(fdstubsfe, hints.fileprefix, "stubsfe", package, True,
	       hints.c_suffix, True, True, c_namespace);
    head_gen_C(fdstubsbe, hints.fileprefix, "stubsbe", package, True,
	       hints.c_suffix, True, True, c_namespace);
    head_gen_C(fdmthfe, hints.fileprefix, "mthfe-skel", package, False,
	       hints.c_suffix, True, True, c_namespace);
    skel_comment(fdmthfe, hints.fileprefix, "mthfe", False);
    pack_init(fdmthfe, package);
    head_gen_C(fdmthbe, hints.fileprefix, "mthbe-skel", package, False,
	       hints.c_suffix, True, True, c_namespace);
    ...
    */
    time_t t;
    time(&t);
    const char *sopath = eyedb::getConfigValue("sopath");
    if (!sopath)
    ...
    if (r) *r = 0;
    fprintf(fdmk, mkpattern, package, package,
	    "Generated by eyedbodl at",
	    ctime(&t), package, package, package,
	    package, package, package, package, package, package,
	    package, package, package, xsopath);
