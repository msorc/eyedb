					     ...
					     DBAccessMode dbmode)
{
  ...
  dbaccess->user(user);
  dbaccess->dbentry(dbentry);
  dbaccess->mode(dbmode);
...
#define check_sysaccess(s, mode)  \
if ((mode) != NoSysAccessMode && \
    (mode) != DBCreateSysAccessMode && \
    (mode) != AddUserSysAccessMode && \
    (mode) != DeleteUserSysAccessMode && \
    (mode) != SetUserPasswdSysAccessMode && \
    (mode) != AdminSysAccessMode && \
    (mode) != SuperUserSysAccessMode) \
					   ...
					   const char *passwdauth)
{
  ...
  check_sysaccess(IDB_SET_USER_SYSACCESS_ERROR, mode);
