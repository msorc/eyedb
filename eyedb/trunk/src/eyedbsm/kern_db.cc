...
#define PR "dbCreate: "
...
#ifdef SEXDR
  h2x_dbHeader(&dbh, &dbh);
#endif
  if ((n = write(dbfd.dbsfd, (char *)&dbh, sizeof(dbh))) != sizeof(dbh)) {
    pop_dir(pwd);
  ...
  pop_dir(pwd);
  pdbh->vd->dbs_addr->state = OPENED_STATE;
...
#define PR "dbDelete: "
  ...
  unsigned int ndat = x2h_u32(dbh->vd->dbs_addr->__ndat);
  for (i = 0; i < ndat; i++)
    {
      if (*dbh->vd->dbs_addr->dat[i].file) {
	if (unlink(dbh->vd->dbs_addr->dat[i].file) < 0) {
	  pop_dir(pwd);
	  return fcouldnot(PR, "unlink", dbh->vd->dbs_addr->dat[i].file);
	}

	if (unlink(dmpfileGet(dbh->vd->dbs_addr->dat[i].file)) < 0) {
	  pop_dir(pwd);
	  return fcouldnot(PR, "unlink", dmpfileGet(dbh->vd->dbs_addr->dat[i].file));
	}
      }
    }
...
#define PR "dbInfo: "
  ...
  if (se = syscheckn(PR, read(fd, (char *)&dbh, sizeof(dbh)), sizeof(dbh),
			"reading database file: '%s'", dbfile))
    return se;

#ifdef SEXDR
  x2h_dbHeader(&dbh, &dbh);
#endif
  ...
  for (int i = 0; i < dbh.__ndsp; i++)
    {
      ...
      DataspaceDesc *dsd = &dbh.dsp[i];

      strcpy(ds->name, dsd->name);
      ds->ndat = dsd->__ndat;
      memcpy(ds->datid, dsd->__datid, sizeof(short) * dsd->__ndat);
    }

  for (int i = 0; i < dbh.__ndat; i++)
    {
      Datafile *df = &info->dat[i];
      DatafileDesc *dfd = &dbh.dat[i];
			  ...
			  const char *, const char *, int))
{
  ...
  for (i = 0; i < dbc->ndat; i++)
    {
      char *from = makefile(fromdbdir, dbh->vd->dbs_addr->dat[i].file);
      char *to = makefile(todbdir, dbc->dat[i].file);

      if (strcmp(from, to))
	{
	  if (s = mvcp(dbh->vd->dbs_addr->dat[i].file,
	  ...
	  if (s = mvcp(dmpfileGet(dbh->vd->dbs_addr->dat[i].file),
	...
	return syserror("rewing database file '%s'", dbfile);
      ...
      if (write(fd, db_header, sizeof(DbHeader)) != sizeof(DbHeader))
	      ...
	      int flag)
{
  ...
  for (i = 0; i < dbc->ndat; i++)
    {
      unsigned int maxsize = x2h_u32(dbh->vd->dbs_addr->dat[i].__maxsize);
		...
		const DbMoveDescription *xdmv, int flag)
{
  ...
  DbHeader db_header;
  Status s;
  register DbCreateDescription *dbc = &dmv.dcr;
  int i, fd;
  Status (*mvcp)(const char *, const char *, 
		    const char *, const char *, int);
  ...
  if ((fd = open(dmv.dbfile, O_RDWR)) >= 0)
    {
      if (read(fd, &db_header, sizeof(DbHeader)) != sizeof(DbHeader))
...
#define PR "dbRelocate: "
  ...
  */
  ...
  h2x_dbHeader(&dbh, &dbh);
#endif
  if (se = syscheckn(PR, write(fd, &dbh, sizeof(dbh)), sizeof(dbh), ""))
    return se;
...
#define PR "dbOpen: "
  ...
  if (se = syscheckn(PR, read(hdfd, dbh, sizeof(*dbh)), sizeof(*dbh), ""))
    {
      free(vd);
      free(dbh);
      return se;
  ...
  *pdbh = (DbHandle *)calloc(sizeof(DbHandle), 1);
  ...
  if ((*pdbh)->vd->dbs_addr->state == OPENING_STATE) {
    pop_dir(pwd);
    return Success;
  }
...
#define PR "dbClose: "
  ...
  for (i = 0; i < ndat; i++)
    {
      register MmapDesc *mmd,
	*mmend = &dbh->vd->dmd[i].mmd[MAX_MMAP_SEGMENTS];

      if (dbh->vd->m_dmp[i]) {
	int mtype = x2h_u16(dbh->vd->dbs_addr->dat[i].mp.mtype);
	int nslots = x2h_u32(dbh->vd->dbs_addr->dat[i].mp.nslots);
  ...
  unsigned int nbobjs = x2h_u32(dbh->vd->dbs_addr->__nbobjs);
...
{
  DbHeader *dbs_addr = DBSADDR(dbh);
  Oid::NX curobj_cnt = 0;

  unsigned int ndat = x2h_u32(dbs_addr->__ndat);
  for (int datid = 0; datid < ndat; datid++) {
    if (isDatValid(dbh, datid)) {
      const MapHeader *mp = DAT2MP(dbh, datid);
#ifdef SEXDR
      curobj_cnt += x2h_u32(mp->BMSTAT.obj_count);
#else
      curobj_cnt += mp->BMSTAT.obj_count;
#endif
    }
  }
...
nsFileSizeExtends(DbHandle const *dbh, short datid, NS curb)
{
  ...
  const char *file = dmpfileGet(DBSADDR(dbh)->dat[datid].file);
...
#error "NO_FILE_LOCK not defined"
  ...
  */
  ...
  dbs_addr->__nbobjs = h2x_u32(maxobjs);
...
{
  return (datid >= 0 && datid < x2h_u32(DBSADDR(dbh)->__ndat) &&
