...
#if 0
  if (vd && vd->semkeys[0])
    printf("mp->id=%d key=%d\n", mp->id, vd->semkeys[0]);
#endif
  mp->plocked = (vd ? &vd->locked : 0);
#endif
#ifdef THR_TRACE
#ifdef UT_SEM
  IDB_LOG(IDB_LOG_MTX,
...
#endif
#if 0
  printf("%d: mutexLightInit(mp=%p, name=%s)\n", getpid(), mp,
	 (mp->pmp && mp->pmp->mtname ? mp->pmp->mtname : "<NIL>"));
#endif
...
#endif
#if 0
  printf("%d: mutexInit(mp=%p, pmp=%p, name=%s)\n", getpid(), mp, pmp, mtname);
  if (!strcmp(mtname, "xm_alloc")) {
    stop_now();
  }
#endif
