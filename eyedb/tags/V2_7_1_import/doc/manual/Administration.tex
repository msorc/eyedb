...
\item \ttv{eyedbrc}: the \eyedb launcher used to start or stop
...
\bi
\item \ttv{"([\^{}{\bks}"]$|${\bks}")*"}, where the surrounding double
...
recommended.
\item running the program \ttv{eyedbrc} with the {start} option, for
instance: \\
\mbox{ }\ttv{eyedbrc start} \\
\mbox{ }\ttv{eyedbrc start -sv\_port 7763} \\
\mbox{ }\ttv{eyedbrc start -eyedblog `eyegetenv topdir`/log/eyedbd.log}
...
\ee
\ee
Note that the \ttv{rc.eyedb} uses \ttv{eyedbrc start} to launch the
\eyedb server.
...
\ttv{eyedbrc start -sv\_port 8100 /tmp/.eyedb-pipe}\\
...
\ttv{eyedbrc start -sv\_port 8100}\\
\ttv{eyedbrc start -sv\_port /tmp/.eyedb-pipe}\\
\\
...
\ttv{eyedbrc start}\\
is equivalent to:\\
\ttv{eyedbrc start -sv\_port `eyedbgetenv sv\_port`}
\\
\\
...
server processus:
\bi
\item the master server that has been launched using the \ttv{eyedbrc start}
command,
\item two children of the master server, each one coressponding to each
client.
\ei
\subsection{Getting Session Information}
The command \ttv{eyedbrc status} gives all the information about the
...
you must use \ttv{eyedbrc status} with the \ttv{-sv\_port} command line option,
as follows:\\
\ttv{eyedbrc status -sv\_port /tmp/.eyedb-pipe}
...
To stop the \eyedb servers, you need to use the \ttv{eyedbrc stop}
