...
get_datid(DbHandle const *dbh, const char *datfile, short &datid,
	     short *dspid = 0)
{
  assert(x2h_u32(DBSADDR(dbh)->__magic) == MAGIC);
  if (!*datfile)
  ...
  if (is_number(datfile))
	...
	return statusMake(INVALID_DATAFILE, "datafile '%s' not found",
			     datfile);
      if (dspid)
	*dspid = getDataspace(DBSADDR(dbh), datid);
      return Success;
    }

  DbHeader *h = DBSADDR(dbh);
		  ...
		  const char *dat_dest)
{
  ...
  DatType dtype = getDatType(DBSADDR(dbh), datid_src);
  if (dtype == PhysicalOidType)
	      ...
	      DatType dtype)
{
  ...
  int ndat = x2h_u32(DBSADDR(dbh)->__ndat);
  short datid;
...
#define PR "datCreate: "
  ...
  DBFD dbfd;
  Status s = checkDatafile(PR, dbh->dbfile, DBSADDR(dbh), &dbc,
  ...
  if (datid == ndat) {
    ndat++;
    DBSADDR(dbh)->__ndat = h2x_u32(ndat);
  }
	   ...
	   Boolean force)
{
  ...
  int fd;
  short datid;
  register DbHeader *h = DBSADDR(dbh);
	      ...
	      unsigned long long newmaxsize)
{
  ...
  */
  ...
  if (nslots < mp->BMH.slot_lastbusy)
    return statusMake(INVALID_DATAFILEMAXSIZE,
			 ...
			 dfd->file,
			 SLOT2KB(mp->BMH.slot_lastbusy, mp->sizeslot));
...
ESM_datDelete(DbHandle const *dbh, const char *datfile, Boolean force)
{
  ...
  short dspid = getDataspace(DBSADDR(dbh), datid);
...
#ifdef SEXDR
  if (!force && x2h_u32(mp->BMSTAT.busy_slots))
#else
  if (!force && mp->BMSTAT.busy_slots)
#endif
...
ESM_datRename(DbHandle const *dbh, const char *datfile, const char *name)
{
  ...
  s = checkNewDatafile(DBSADDR(dbh), "", name);
  if (s) return s;

  strcpy(DBSADDR(dbh)->dat[datid].name, name);
...
ESM_datDefragment(DbHandle const *dbh, const char *datfile)
{
  ...
  strcpy(DBSADDR(dbh_n)->dat[datid].file, datfile_keep);
  strcpy(DBSADDR(dbh_n)->dat[datid].name, datname_keep);
  *DBSADDR(dbh_n)->dat[tmp_datid].file = 0;
  *DBSADDR(dbh_n)->dat[tmp_datid].name = 0;

  // added the 20/03/02
  LASTNSBLKALLOC(dbh_n, datid) = LASTNSBLKALLOC(dbh_n, tmp_datid);

  unsigned int ndat = x2h_u32(DBSADDR(dbh_n)->__ndat);
  if (tmp_datid == ndat - 1) {
    ndat--;
    DBSADDR(dbh_n)->__ndat = h2x_u32(ndat);
  }
...
ESM_datGetInfo(DbHandle const *dbh, const char *datfile, DatafileInfo *info)
{
  ...
  info->datid = datid;
  info->datafile.dspid = getDataspace(DBSADDR(dbh), datid);
  ...
  info->objcnt = mp->BMSTAT.obj_count;
  info->totalsize = mp->BMSTAT.busy_size;
  info->avgsize = (mp->BMSTAT.obj_count ?
		   mp->BMSTAT.busy_size/mp->BMSTAT.obj_count : 0);

  info->busyslotcnt = mp->BMSTAT.busy_slots;
  info->lastbusyslot = mp->BMH.slot_lastbusy;
  info->lastslot = x2h_u32(dat->__lastslot);
  info->busyslotsize = (unsigned long long)mp->BMSTAT.busy_slots * mp->sizeslot;
  ...
  //info->totafilesize = (unsigned long long)mp->BMH.slot_lastbusy * mp->sizeslot;

  info->curslot = mp->BMH.slot_cur;
  if (!mp->BMH.slot_cur)
    info->defragmentablesize = 0;
  else
    info->defragmentablesize = (((unsigned long long)mp->BMH.slot_lastbusy+1 - mp->BMSTAT.busy_slots) * mp->sizeslot);
  ...
  info->used = ((double)mp->BMSTAT.busy_slots/(double)mp->nslots)*100.;
